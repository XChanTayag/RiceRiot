		...
		if (value == 0) 
		{
			if (!facingLeft) {
				if (Time.time > nextFire) {
					...
					tmp.GetComponent<EnemyKnife> ().Initialize (Vector2.right);
				}
			} 
			else 
			{
				if (Time.time > nextFire) {
					nextFire = Time.time + fireRate;
					GameObject tmp = (GameObject)Instantiate (knifePrefab, transform.position, Quaternion.Euler (new Vector3 (0, 0, 90)));
					tmp.GetComponent<EnemyKnife> ().Initialize (Vector2.left);
				}
			}
		}
